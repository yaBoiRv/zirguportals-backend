            ...
            try {
                ...
                for (const [recipientId, msg] of recipients) {
                    ...
                    try {
                        ...
                        const shouldSend = recipient?.email && (prefs.forum_replies_email ?? prefs.forum_replies ?? false) === true;

                        // Check explicit false (default true)
                        if (shouldSend) {
                            const linkText = getTranslation(lang, 'view_topic');
                            const link = `${process.env.APP_WEB_URL}/${lang}/forums/${id}#reply-${replyRecord.id}`;
